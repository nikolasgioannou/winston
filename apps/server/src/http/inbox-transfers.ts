...
import { inboxTransferSchema } from "@winston/contracts/artifacts";
...
export function createInboxTransferGroup(
  database: Pick<ReturnType<typeof createDatabase>, "authenticateInboxTransfer">,
) {
  const token = (request: Request) =>
    request.headers.get("Authorization")?.match(/^Bearer (\S+)$/)?.[1] ?? "";
  const router = new Hono<HttpEnvironment>();
  router.post("/inbox/authorize", async (context) => {
    const identity = context.get("identity");
    if (identity.kind !== "transfer") throw new RequestError("unauthorized");
    const expected = await parseJson(context, inboxTransferSchema);
    const current = await database.authenticateInboxTransfer(token(context.req.raw));
    if (
      !current ||
  ...
  return {
    ...
    async authenticate(request: Request): Promise<Identity | null> {
      if (
        request.method !== "POST" ||
        new URL(request.url).pathname !== "/api/transfers/inbox/authorize"
      )
        return null;
      const current = await database.authenticateInboxTransfer(token(request));
      return current
