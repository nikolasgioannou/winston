...
export function createInboxHandler(options: {
  ...
  return async (request: Request): Promise<Response | null> => {
    if (new URL(request.url).pathname !== "/v1/inbox") return null;
    ...
    try {
      ...
      transfer = inboxTransferSchema.parse(
    ...
    try {
      ...
      await request.body?.cancel().catch(() => undefined);
      return reply(503, { error: "inbox_unavailable" });
    } finally {
      active -= 1;
    }
  };
