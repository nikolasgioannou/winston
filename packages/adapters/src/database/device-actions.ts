...
import { deviceOperationSchema, type DeviceFileSource } from "@winston/contracts/devices";
...
export function deviceActionRepository(transaction: DatabaseTransaction, ownerId: string) {
  async function prepare(
    ...
    input:
      | { kind: "operation"; value: unknown }
      | { kind: "file.read"; path: string }
      | { kind: "file.write"; path: string; overwrite: boolean; source: DeviceFileSource },
  ) {
    ...
    const requested =
      input.kind === "operation"
        ...
        : input.kind === "file.read"
          ? deviceOperationSchema.options[1].omit({ transferId: true }).parse(input)
          : deviceOperationSchema.options[2].omit({ transferId: true }).parse(input);
    ...
    const operation = deviceOperationSchema.parse(
      input.kind !== "operation"
        ? {
            ...requested,
            transferId:
              prior?.success &&
              (prior.data.kind === "file.read" || prior.data.kind === "file.write") &&
              prior.data.kind === input.kind
  ...
  return {
    ...
    prepareFileWrite(
      ...
      return prepare(task, key, deviceId, { kind: "file.write", path, overwrite, source });
    },
  };
}
