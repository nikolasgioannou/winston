...
Local API, Telegram polling, and migration entrypoints reject remote PostgreSQL URLs before connecting. Use explicit loopback hosts, users, and database names; application and direct URLs must use the same local database and port. Local connection parameters are limited to `sslmode`, `application_name`, and `connect_timeout`. Web/auth origins must use loopback HTTP locally and public HTTPS in production. Sign-in and Google connections require separate OAuth client IDs. These checks prevent mixed-target configuration mistakes; they cannot identify the provenance of arbitrary provider keys or detect a manually forwarded production database. Use dedicated development credentials and storage. Production migrations require explicit `NODE_ENV=production`, which the production container supplies.
...
The browser journal binds its control files and profile to an explicit owner/browser identity. Initialization requires new storage; reopening validates the identity, schema, ownership, and permissions. Consecutive epochs are committed with SQLite WAL and full synchronization. Replaced directories, symlinks, hard-linked journals, and stale writes are rejected. The service must acquire the exclusive profile lock before opening this journal and must stop the previous browser process before recovery; storage alone does not enforce process ownership.

The browser container entrypoint takes that kernel lock on `/data` and runs a single broker with headed Chromium and a Unix-socket-only Xvfb display. Chromium runs as UID 1000 with its sandbox enabled and a minimal environment; broker state stays root-owned. Set `BROWSER_OWNER_ID` and `BROWSER_ID`, initialize a new mounted volume once with `--initialize`, then start normally. Build the `runtime` Docker target for deployment; `validation` contains synthetic test fixtures. Only `GET /health` is exposed. Restart the entire container or Machine after failure, never just the broker inside a surviving process tree. Fly's init terminates the Machine when its entrypoint exits, as described in [Fly's process guidance](https://fly.io/docs/app-guides/multiple-processes/). Fly-specific sandbox and network validation remains required before deployment.

`bun run test:browser-runtime` builds disposable Linux containers on the local Docker engine and checks persistent synthetic cookies, exclusive locking, process privileges, credential isolation, closed control ports, and broker crash recovery. It mounts no host profile and publishes no ports. Chromium and display packages are installed inside the image. The test seccomp profile is adapted from [Playwright 1.63.0](https://github.com/microsoft/playwright/blob/v1.63.0/utils/docker/seccomp_profile.json), permitting `chroot` inside Chromium's user namespace without granting container-level `SYS_CHROOT` or `SYS_ADMIN`. Kernel capability checks still apply, and the fixture checks Chromium's namespace and seccomp sandbox status. Revalidate the image when upgrading Playwright or Debian Chromium.
...
`test` runs tooling, unit, PostgreSQL integration, and browser suites without Google, OpenRouter, or other production credentials. A stopped container runtime is a failure, not a skipped integration test. On macOS, integration tests also connect the real Swift command runtime to the authenticated HTTP CLI and database-backed device socket, checking output, duplicate requests, and cancellation with disposable processes and identities. This native integration is skipped on Linux; it requires the installed Swift toolchain and local container runtime. It does not use Keychain, pair the installed app, or request privacy permissions. Deterministic adapters do not replace real-provider smoke checks: OAuth grants and refresh, model streaming/tool calls, cloud browser takeover, and macOS permissions must each be verified against their real services or operating system before those integrations are considered complete.
